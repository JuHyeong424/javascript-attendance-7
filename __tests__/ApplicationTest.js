...
describe("테스트", () => {
  ...
  test("주말 또는 공휴일 예외 테스트", async () => {
    mockNowDate("2026-01-03");

    await runExceptions({
      inputs: ["1"],
      inputsToTerminate: INPUTS_TO_TERMINATE,
      expectedErrorMessage: "[ERROR] 01월 03일 토요일은 등교하는 날이 아닙니다.",
    });
  });

  test("출석 확인 기능 테스트", async () => {
    mockNowDate("2026-01-07");

    await run({
      inputs: ["1", "짱수", "10:00"],
      inputsToTerminate: INPUTS_TO_TERMINATE,
      expected: ["01월 07일 수요일 10:00 (출석)"],
    });
  });
  ...
  test("출석 수정 및 크루별 출석 기록 확인 기능 테스트", async () => {
    ...
    await run({
      inputs: ["2", "짱수", "2", "10:00", "3", "짱수"],
      inputsToTerminate: INPUTS_TO_TERMINATE,
      expected: [
        "01월 02일 금요일 13:00 (결석) -> 10:00 (출석) 수정 완료!",

        "이번 달 짱수의 출석 기록입니다.",
        "01월 01일 목요일 --:-- (결석)",
        "01월 02일 금요일 10:00 (출석)",
        "01월 05일 월요일 10:00 (출석)",
        "01월 06일 화요일 10:00 (출석)",
        "01월 07일 수요일 --:-- (결석)",
